...
export const ErrorStatsScreen: React.FC<ErrorStatsScreenProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    try {
      const s = ErrorLogger.getErrorStats();
      setStats(s ?? null);
    } catch (err) {
  ...
  const handleClear = () => {
    if (!confirm('هل أنت متأكد من مسح جميع السجلات والإحصائيات؟')) return;
    try {
  ...
  if (!stats) {
    return (
      <div className="flex-1 flex items-center justify-center p-6">
        <p className="text-gray-500">جارٍ تحميل الإحصائيات...</p>
      </div>
    );
  }

  // تحويل الإحصائيات إلى قائمة مع استبعاد last_error_time
  const { last_error_time, ...rest } = stats as any;
  const statsList = Object.entries(rest);
  const lastErrorTime = typeof last_error_time === 'number' ? last_error_time : 0;

  const locale = typeof navigator !== 'undefined' ? navigator.language || 'ar-DZ' : 'ar-DZ';
    ...
    <div className="flex flex-col h-full bg-[#E65100] dark:bg-zinc-950 text-black dark:text-white transition-colors duration-500">
      ...
      <div className="flex items-center justify-between p-6 bg-black/10 backdrop-blur-sm sticky top-0 z-10 border-b border-white/10">
        <div className="flex items-center gap-3">
          {onBack && (
            <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full transition-colors text-white" aria-label="رجوع">
        ...
        {activeTab === 'errors' ? (
            ...
            {lastErrorTime > 0 && (
              ...
              >
                ...
                <div>
                  ...
                  <p className="font-bold text-black dark:text-white">{new Date(lastErrorTime).toLocaleString(locale)}</p>
                </div>
              </motion.div>
            )}
          ...
          <div className="space-y-6">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-3">
                {registries.length === 0 ? (
                  <p className="text-white/40 px-2">لم يتم فحص المستودعات بعد.</p>
                ) : (
